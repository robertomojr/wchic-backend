...
import { syncLeadToPodio, updateLeadStatusInPodio } from "../services/podioSyncService.js";
...
/**
 * PATCH /leads/:id/status — Atualiza status no banco e propaga ao Podio
 *
 * Body: { status: "closed" }
 */
leadsRouter.patch("/:id/status", async (req, res) => {
  ...
  if (!status) {
    return res.status(400).json({ ok: false, error: "Envie { status }" });
  }

  const check = await query("SELECT id FROM leads WHERE id = $1", [id]);
  if (!check.rows[0]) {
  ...
  // Propaga ao Podio de forma não-bloqueante
  updateLeadStatusInPodio(id, status).catch((err) => {
    console.error("Erro ao atualizar status no Podio:", err?.message);
  });

  return res.json({ ok: true, lead_id: id, status });
});

/**
 * POST /leads/:id/sync-podio — Dispara sync manual completo para o Podio
 */
